...
The phase offset was critical for the calculation in Method 1, thus an incorrect offset would give us andincorrect value for the Verdet constant.
...
\section{Conclusion}
{We observed the Faraday effect with our experiment. We found a magnetic field dependent shift in the polarization of light as it propagated through the glass rod. 
The Verdet constant for the rod was calculated from two experimental methods.  The first method was likely affected by a systematic error in our data recording and ended up being $17.54 \pm 1.12 \mathrm{~rad/T} \cdot \textrm{m}$, not within agreement of the values calculated by our colleagues. 
The second method gave us a constant of $19.9 \pm 1.0 \mathrm{~rad/T} \cdot \textrm{m}$, which is in good agreement with the values our colleagues calculated.  The difference between our two calculated values is about $10\%$ larger than the error, thus the disagreement is significant.

In order to eliminate systematic error, we would want to repeat this experiment with another polarizer, as we suspect we used a defective polarizer unknowingly, which had incorrectly applied angle marlin
}
